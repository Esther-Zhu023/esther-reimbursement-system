...
const CONFIG = {
  MAX_AMOUNT_PER_REQUEST: 150,   // RMB
...
async function submitReimbursement({
  ...
  // 发票附件验证：人工提交必须上传 PDF 发票
  if (submitterType === 'human' && (!attachments || attachments.length === 0)) {
    return { success: false, error: '必须上传 PDF 发票' };
  }
  ...
  if (monthlyCount >= CONFIG.MAX_REQUESTS_PER_MONTH) {
    return { success: false, error: `本月报销额度已用完（最多 ${CONFIG.MAX_REQUESTS_PER_MONTH} 张凭证）` };
  }
...
async function rejectReimbursement(id, approverId, approverName, reason) {
  ...
  if (currentCount > 0) {
    await store.set(countKey, currentCount - 1);
  }
...
async function getDashboardStats() {
  ...
  const pendingAgent = pending.filter(r => r.submitterType === 'agent').length;
  const pendingHuman = pending.filter(r => r.submitterType === 'human').length;
